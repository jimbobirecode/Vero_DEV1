// The guide's content, checked against the Club Vero dashboard build in this
// repository (vero-dashboard.html) rather than against the earlier draft.
module.exports = {
  edition: 'August 2026',
...

    {
      ...
      articles: [
        ...
        },
        {
          ...
          extraCaption: 'Overview → Outlets gives the same period as a status per room.',
          steps: [
      ...
      articles: [
        {
          ...
          extraCaption: 'Further down the same screen: the tee sheet upload, the golf survey log, and the golf questions themselves.',
          steps: [
...

    {
      ...
      articles: [
        ...
        },
        {
          ...
          extraCaption: 'Further down: survey delivery, weekly analysis, the per-member cap, and staff shift surveys.',
          steps: [
  ...
  troubleshooting: [
    ['A member did not receive a survey',
     'People → Visits to confirm the visit qualified, then Communications → Survey Queue for a blocked reason, then Communications → Message Log for the delivery attempt. Why hasn’t anything sent? on the queue checks all of it at once.'],
    ['Nothing at all is sending',
     'Communications → Survey Queue → Why hasn’t anything sent?. It checks contact details, provider configuration, send time and SMS credit and names the one that is failing.'],
    ['A survey was sent but no response is showing',
     'Communications → Survey Log for the status, and Communications → Message Log for whether the message itself was delivered or failed.'],
    ['A poor score needs following up',
     'Intelligence → Case Alerts. Assign an owner, call the member, Log call, then Resolve once the action is complete.'],
    ['Case alerts are going to the wrong person',
     'Setup → Settings → Outlets, and set Alerts go to for that outlet.'],
    ['Golf surveys are not appearing',
     'Overview → Golf. Check the tee sheet upload read the member numbers and the date, then check the Golf survey log.'],
    ['A server is not receiving shift surveys',
    ...
    ['A number in a report does not match the dashboard',
     'Check the period. The report is computed once on the server for the period chosen, and the dashboard defaults to the last 7 days.'],
  ],
};
